const Cart = () => {
  ...
  const shop_currency_rate = Shopify.currency.rate || 1;
  const moneyFormat = window.themeCore.objects.shop.money_format;
  const Accordion = window.themeCore.Accordion;
  const selectors = {
  ...
  function setupTermsAndConditions() {
    ...
    if (!termsAndConditionsCheckbox || cartFooterButtonMessage && cartFooterButtonMessage.classList.contains(cssClasses.disabled)) {
      return;
    }
